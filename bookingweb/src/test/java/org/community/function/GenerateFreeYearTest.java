...
public class GenerateFreeYearTest {
    ...
    public static void generateData(Jdbi jdbi, LocalDateTime currentTimestamp) {
        jdbi.useHandle(handle -> {
            ...
            for (int i = 1; i <= 10; i++) {
                ...
                userBatch.bind(0, 1000 + i).bind(1, "user_" + i).bind(2, jsonAddress).add();
            }
            userBatch.execute();
            List<Long> userIds = handle.createQuery("SELECT id FROM user ORDER BY id ASC").mapTo(Long.class).list();
            ...
            long assetId = handle.createQuery("SELECT id FROM asset ORDER BY id ASC LIMIT 1").mapTo(Long.class).one();
            ...
            long locationId = handle.createQuery("SELECT id FROM location ORDER BY id ASC LIMIT 1").mapTo(Long.class)
                    .one();
            ...
            long freeId = handle.createQuery("SELECT id FROM free ORDER BY id ASC LIMIT 1").mapTo(Long.class).one();
    ...
    public void testGenerateFreeYear() {
        ...
        jdbi.useHandle(handle -> {
            ...
            String user1Name = handle.createQuery("SELECT name FROM user WHERE id = 1").mapTo(String.class).one();
            assertEquals("user_1", user1Name);
            String user2Name = handle.createQuery("SELECT name FROM user WHERE id = 2").mapTo(String.class).one();
            assertEquals("user_2", user2Name);
            ...
            int locationCount = handle.createQuery("SELECT COUNT(*) FROM location").mapTo(Integer.class).one();
            assertEquals(1, locationCount, "Det ska finnas 1 plats");

            int assetLocationCount = handle.createQuery("SELECT COUNT(*) FROM asset_location").mapTo(Integer.class)
                    .one();
            ...
            String freeStartTime = handle.createQuery("SELECT start_time FROM free LIMIT 1").mapTo(String.class).one();
            String freeEndTime = handle.createQuery("SELECT end_time FROM free LIMIT 1").mapTo(String.class).one();
            ...
            assertTrue(bookedCount >= 100, "Det bör finnas cirka 104 bokningar (52 tisdagar + 52 torsdagar)");
            ...
            for (Models.Booked b : user1Bookings) {
                assertEquals(DayOfWeek.TUESDAY, b.startTime.getDayOfWeek(), "Bokning för user_1 ska vara en tisdag");
                ...
                assertTrue(!b.startTime.isBefore(now), "Bokning ska inte starta före current timestamp");
            ...
            for (Models.Booked b : user2Bookings) {
                assertEquals(DayOfWeek.THURSDAY, b.startTime.getDayOfWeek(), "Bokning för user_2 ska vara en torsdag");
                assertEquals(18, b.startTime.getHour(), "Starttid ska vara 18:00");
                assertEquals(0, b.startTime.getMinute(), "Startminut ska vara 00");
                assertEquals(20, b.endTime.getHour(), "Sluttid ska vara 20:00");
                assertEquals(0, b.endTime.getMinute(), "Slutminut ska vara 00");
